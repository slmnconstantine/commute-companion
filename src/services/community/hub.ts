...
import { createNotification } from '@/services/communication/notifications';
...
export const getPosts = async (
  ...
  // Filter out any posts reported by more than 20 unique users
  let safePosts = posts;
  if (postIds.length > 0) {
    try {
      const { data: reportsData } = await supabase
        .from('reports')
        .select('post_id, reporter_id')
        .in('post_id', postIds);

      if (reportsData && reportsData.length > 0) {
        const reportersMap: Record<string, Set<string>> = {};
        reportsData.forEach((r: { post_id: string; reporter_id: string }) => {
          if (r.post_id && r.reporter_id) {
            if (!reportersMap[r.post_id]) reportersMap[r.post_id] = new Set();
            reportersMap[r.post_id].add(r.reporter_id);
          }
        });
        safePosts = posts.filter((p) => (reportersMap[p.id]?.size || 0) <= 20);
      }
    } catch (e) {
      console.warn('Reports check note in getPosts:', e);
    }
  }
